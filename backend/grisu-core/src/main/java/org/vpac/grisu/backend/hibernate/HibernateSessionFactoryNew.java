...
public final class HibernateSessionFactoryNew {
	
	private HibernateSessionFactoryNew() {
	}
		...
		try {
			...
			if (grisuHibernateConfigFile.exists()) {
				...
				if ("mysql".equals(dbType)) {
					...
					if (url == null || url.length() == 0) {
						url = "jdbc:hsqldb:file:" + Environment.GRISU_DIRECTORY
				...
				configuration = new AnnotationConfiguration()
						.configure("/grisu-hibernate-default-hsqldb.cfg.xml");
				String url = "jdbc:hsqldb:file:" + Environment.GRISU_DIRECTORY
						+ File.separator + "grisulocaldb";
				configuration.setProperty("hibernate.connection.url", url);
				configuration
