...
public class GT5Submitter extends JobSubmitter{
    ...
    @Override
    protected String submit(InformationManager infoManager, String host, String factoryType, Job job) {
        String rsl = createJobSubmissionDescription(infoManager,job.getJobDescription());
        System.out.println("RSL is ... " + rsl);
     ...
     private String createJobSubmissionDescription(
			final InformationManager infoManager, final Document jsdl) {
                ...
                NameOpValue executable = new NameOpValue("executable",NameOpValue.EQ,JsdlHelpers.getPosixApplicationExecutable(jsdl));

                String[] argumentsVal = JsdlHelpers.getPosixApplicationArguments(jsdl);
                NameOpValue arguments = null;
                if (argumentsVal != null){
                    arguments = new NameOpValue("arguments",NameOpValue.EQ,argumentsVal);
                }

                NameOpValue stdout = new NameOpValue("stdout",NameOpValue.EQ,JsdlHelpers.getPosixStandardOutput(jsdl));
                NameOpValue stderr = new NameOpValue("stderr",NameOpValue.EQ,JsdlHelpers.getPosixStandardError(jsdl));

                String inputVal = JsdlHelpers.getPosixStandardInput(jsdl);
                NameOpValue stdin = null;
                if (inputVal != null){
                    stdin = new NameOpValue("stdin",NameOpValue.EQ,JsdlHelpers.getPosixStandardInput(jsdl));
                }

                String dirValue = JsdlHelpers.getWorkingDirectory(jsdl);
                NameOpValue directory = null;
                if (dirValue != null && !"".equals(dirValue.trim())){
                    directory = new NameOpValue("directory",NameOpValue.EQ,JsdlHelpers.getWorkingDirectory(jsdl));
                }
		...
		if (processorCount > 1) {
			...
			if (jobTypeString == null) {
				jobType = new NameOpValue("job_type",NameOpValue.EQ,"mpi");
                            ...
                            jobType = new NameOpValue("job_type",NameOpValue.EQ,jobTypeString);
			}
		} else {
			if (jobTypeString == null) {
		...
		if ((memory != null) && (memory >= 0)) {
			...
			memory = memory / 1024;
			maxMemory = new NameOpValue("max_memory",NameOpValue.EQ,""+memory);
		}
		...
		if (walltime > 0) {
			...
			// convert to minutes
			wt = wt / 60;
                        maxWalltime = new NameOpValue("max_wall_time",NameOpValue.EQ,""+wt);
		}
                ...
                if (emailAddressVal != null ){
                    emailAddress = new  NameOpValue("email_address",NameOpValue.EQ,emailAddressVal);
                    if (onStart){
                        emailStart = new  NameOpValue("email_on_execution",NameOpValue.EQ,"yes");
                    }
                    if (onFinish){
                        emailAbort = new  NameOpValue("email_on_abort",NameOpValue.EQ,"yes");
                        emailTermination = new  NameOpValue("email_on_termination",NameOpValue.EQ,"yes");
                    }
                }
                ...
                if (modulesVal != null) {
                    for (String moduleStr: modulesVal){
                        NameOpValue module = new  NameOpValue("module",NameOpValue.EQ,moduleStr);
