...
public final class HibernateSessionFactory {
	...
	/**
	 * Gets a session for a user. Every dn get's its own session. Sessions are
	 * managed in a static map using the dn as key and the session as value.
	 * 
	 * @param dn
	 *            the dn of the user
	 * @return the hibernate session of the user
	 */
	public static Session getSession(final String dn) {
	...
	public static void rebuildSessionFactory() {
		try {
			...
			if (configuration.getProperty("dialect").equals(
					"org.hibernate.dialect.HSQLDialect")) {
				configuration.setProperty("hibernate.connection.url",
						"jdbc:hsqldb:file:" + System.getProperty("user.home")
								+ "/.grisu/grisulocaldb");
				myLogger
						.debug("hsql dialect found. Changed settings to use database file "
