...
public class GrisuJob extends JobDescription implements
    ...
    public void createAndSubmit(String group, int retries, int secondsInbetweenRetries) throws JobSubmissionException {

        boolean created = false;
        Exception lastException = null;
        for (int i = 0; i < retries; i++) {
            try {
                myLogger.debug("Creating job, {}. try.", Integer.toString(i+1));
                createJob(group);
                created = true;
                break;
            } catch (Exception e) {
                lastException = e;
                myLogger.error("Job creation failed, {}. try: {}", Integer.toString(retries+1), e.getLocalizedMessage());
                try {
                    Thread.sleep(secondsInbetweenRetries * 1000);
        ...
        if (!created) {
            throw new JobSubmissionException("Could not create job.", lastException);
        }
        ...
        for (int i = 0; i < retries; i++) {
            try {
                ...
                myLogger.debug("Submitting job, {}. try.", Integer.toString(i+1));
                submitJob(true);
                submitted = true;
                break;
                ...
                lastException = e;
                myLogger.error("Job submission failed, {}. try: {}", Integer.toString(retries+1), e.getLocalizedMessage());
                try {
                    Thread.sleep(secondsInbetweenRetries * 1000);
                } catch (InterruptedException e1) {
                    throw new RuntimeException(e1);
    ...
    public File downloadOutput(String path, int retries, int secondsInbetweenRetries) throws RemoteFileSystemException {
        ...
        for (int i = 0; i < retries; i++) {
            try {
                ...
                lastException = e;
                myLogger.debug("Download failed, {}. try: {}", Integer.toString(retries+1), e.getLocalizedMessage());
                try {
                    Thread.sleep(secondsInbetweenRetries+1000);
                } catch (InterruptedException e1) {
                    throw new RuntimeException(e1);
        ...
        if ( ! downloaded ) {
            throw new RemoteFileSystemException("Download failed after "+retries+" tries.", lastException);
        }
     ...
     *
     * @param si            the serviceInterface
     * @param jobProperties the basic properties of the job (no. of cpus, application to
     *                      use, ...)
     */
    public GrisuJob(final ServiceInterface si,
     ...
     * @throws NoSuchJobException if there is no job with the specified name on the backend
     ...
     * @param si                        the serviceInterface
     * @param jobname                   the name of the job
     * @param refreshJobStatusOnBackend whether to refresh the status of the job on the backend
     * @throws NoSuchJobException if there is no job with the specified name on the backend
     *                            connected to the specified serviceInterface
     */
    public GrisuJob(final ServiceInterface si, final String jobname,
     ...
     * @throws JobPropertiesException    if the job can't be archived
     * @throws RemoteFileSystemException if the job can't be archived because the target url can't be
     *                                   accessed
     */
     ...
     *
     * @param target the target url to archive the job or null to use the default
     *               target
     * @return the url of the target archived jobdirectory
     * @throws JobPropertiesException    if the job can't be archived
     * @throws RemoteFileSystemException if the job can't be archived because the target url can't be
     ...
     * Archives the job.
     *
     * @param target                   the url (of the parent dir) to archive the job to or null to
     *                                 use the (previously set) default archive location
     * @param waitForArchivingToFinish whether to wait until archiving is finished
     * @throws JobPropertiesException    if the job is not finished yet
     * @throws RemoteFileSystemException if the archive location is not specified or there is some
     ...
     *         GrisuJob object).
     * @throws JobPropertiesException if one of the properties is invalid and the job could not be
     *                                created on the backend
     */
...
//				.getUserEnvironmentManager().getCurrentFqan();
     ...
     * {@link #createJob(String, String)} method.
     *
     * @param forceRefresh whether to forcefully refresh the job properties
     * @return the job properties
     */
...
//			}
     ...
     *
     * @param relatevePathToJobDir the path to the file relative to the job directory.
...
//        }
     ...
     *
     * @param key          the key
     * @param forceRefresh whether to refresh the job property forcefully or not
     * @return the property
     */
     ...
     * Returns the job log
     *
     * @param forceRefresh whether to forcefully refresh the log messages
     * @return the job log
     */
     ...
     *
     * @param forceRefresh whether to use the cached status (false) or force a status
     *                     refresh (true)
     ...
     *
     * @param forceRefresh whether to refresh the status of the job on the backend or not
...
//        }
     ...
     *
     * @param jobname the base jobname
     */
    @Transient
     ...
     *
     * @throws FileTransactionException if a file can't be staged in
     ...
     * local machine.
     *
     * @param waitForSubmissionToFinish whether to wait for submission to finish or not
     * @return the handle to the job submission task on the backend
     * @throws JobSubmissionException if the job could not be submitted
     * @throws InterruptedException
     */
     ...
     * local machine.
     *
     * @param additionalJobProperties properties you want to store with the job (only get stored if
     ...
     * @param additionalJobProperties   properties you want to store with the job (only get stored if
     *                                  submission was successful)
     * @param waitForSubmissionToFinish whether to wait for submission task to finish on backend
     ...
     *
     * @param b whether the job is about to be cleaned or not.
     */
