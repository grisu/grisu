...
//			LocalTemplatesHelper.copyTemplatesAndMaybeGlobusFolder();
     ...
     *
     * @param batchJobnameBase the id (name) of the multipartjob
     * @throws JobPropertiesException
     */
     ...
     * @throws NoValidCredentialException if no valid credential could be found to create the user
     */
    ...
    public DtoMessages getMessages() {
        return DtoMessages.createDtoMessages(getUser().getMessages());
    }

    public void removeMessage(String title) {
    }

    public DataHandler downloadSharedFile(String token)
            throws RemoteFileSystemException {
        return null;
    }
    ...
    public String sendMessage(DtoStringList users, DtoMessage message) {
        ...
        List<String> notifiedUsers = Lists.newArrayList();
        for (String user : users.asSortedSet()) {
            myLogger.debug("Adding message '{}' for user '{}", message.getTitle(), user);
            List<User> userobjects = User.userdao.findAllUsersThatContain(user);
            if (userobjects != null) {
        ...
        if (notifiedUsers.size() == 0) {
            msg = "No user matched, nobody notified";
        } else {
            msg = "Notified: " + StringUtils.join(notifiedUsers, ",");
        }
        return msg;
    }
