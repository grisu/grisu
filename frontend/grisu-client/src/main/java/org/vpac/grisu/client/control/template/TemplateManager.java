 ...
 * templates. Local ones that reside in $HOME/.grisu/templates and remote ones which are 
 * delivered via the {@link ServiceInterface}. Should 2 templates with the same name exist,
 * the local one is used. Remote templates are dowloaded every time the grisu client connects
...
public class TemplateManager {
	...
	public TemplateManager(EnvironmentManager em) {
		...
		String[] serverTemplateNames = ClientPropertiesManager.getServerTemplates();
		...
		for ( String serverTemplateName : serverTemplateNames ) {
			try {
				...
				myLogger.error("Could not add server template: "+serverTemplateName);
	...
	public static void main (String[] args) throws ServiceInterfaceException {
		...
		LoginParams params = new LoginParams(
			ClientPropertiesManager.getDefaultServiceInterfaceUrl(), "markus", "xxx".toCharArray(),
...
//		EnvironmentManager.setDefaultServiceInterface(serviceInterface);
		...
		for ( String templateName : tm.getLocalTemplates().keySet() ) {
			...
			System.out.println(JsdlHelpers.getDescription(template.getTemplateDocument()));
...
//			System.out.println(template.getCommandLineOptions());
	...
	public void addServerTemplate(String templateName) {
		...
		try {
			...
			myLogger.error("Could not add server template: "+templateName);
		}
		}
	...
	public void removeLocalTemplate(String templateName) {
		...
		File tempToRemove = new File(LocalTemplateManagement.TEMPLATE_DIRECTORY, templateName+".xml");
	...
	/**
	 * Copies a remote template into the local template store. If a template with the same name already
	 * exists there, a new name is used (name_1.xml, name_2.xml and so on).
	 * 
	 * @param templateName the (remote) template name
	 */
