 ...
 *
 */
public class Common extends AbstractModule implements FqanListener, MountPointsListener, SubmissionObjectHolder {
		...
		if (useMDS) {
			if (currentSitesMDS == null) {
				...
				for (SubmissionLocation loc : getAvailableSubmissionLocationsForCurrentVO()) {
					...
					Set<SubmissionLocation> siteLocs = currentSitesMDS.get(site);
					if ( siteLocs == null ) {
			...
			if (currentSitesNonMDS == null) {
				...
				for (SubmissionLocation loc : allAvailSubLocs) {
					String site = loc.getSite();
					Set<SubmissionLocation> siteLocs = currentSitesNonMDS.get(site);
					if ( siteLocs == null ) {
						siteLocs = new TreeSet<SubmissionLocation>();
	...
	/**
	 * Returns whether there exists a preferred executable type. TemplatePanels for example
	 * can then determine which executable to use. Returns either {@link #EXECUTABLE_TYPE_UNDEFINED}, {@link #EXECUTABLE_TYPE_SERIAL} or {@link #EXECUTABLE_TYPE_PARALLEL}
