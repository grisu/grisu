...
public class UnderworldInputFile extends InputFile {
	...
	private void parseUnderworldInputFile() throws TemplatePreProcessorException {
        ...
        if(!input.startsWith("file:"))
        {
            ...
            try
            {
                input_fileObject = node.getTemplate().getEnvironmentManager().getFileManager().getFileObject(new URI(remote_file));
                if(input_fileObject == null)
                {
                    throw new TemplatePreProcessorException((new StringBuilder()).append("Could not retrieve remote file: ").append(remote_file).toString());
                }
            }
            catch(URISyntaxException e)
            {
                throw new TemplatePreProcessorException((new StringBuilder()).append("Could not create temp file for element because of wrong uri syntax: ").append(remote_file).toString());
            }
            underworld_config_file = input_fileObject.getLocalRepresentation(true);
        } else
        {
        	try {
        		underworld_config_file = new File(new URI(input));
        	} catch (Exception e) {
            		throw new TemplatePreProcessorException("Could not access file.",e);
            }
        }
        ...
        try
        {
            input_doc = SeveralXMLHelpers.fromString(FileHelpers.readFromFile(underworld_config_file));
        }
        catch(Exception e)
        {
            throw new TemplatePreProcessorException("Could not parse input file to get output directory.");
        }
        ...
        node.getTemplate().getJobProperties().put("underworldOutputDirectory", output_dir);
