...
public class BatchJobMonitoringGrid extends JPanel {
	...
	public BatchJobMonitoringGrid(ServiceInterface si, String application) {
		...
		observedBatchJobs = new ObservableElementList<BatchJobObject>(batchJobs, bjoConnector);

		sortedBatchJobList = new SortedList<BatchJobObject>(observedBatchJobs, new BatchJobObjectComparator());
...
//			table.getColumnExt(0).setHeaderRenderer()
			...
			table.setDefaultRenderer(Boolean.class, new BatchJobRefreshRenderer());
			table.setDefaultRenderer(Double.class, new BatchJobStatusCellRenderer());
	...
	private void batchJobDoubleClickOccured() {
		...
		if (selRow >= 0) {
			...
			BatchJobObject sel = (BatchJobObject) batchJobModel.getValueAt(selRow, 1);
	...
	// register a listener
	synchronized public void addBatchJobSelectionListener(BatchJobSelectionListener l) {
		if (listeners == null)
