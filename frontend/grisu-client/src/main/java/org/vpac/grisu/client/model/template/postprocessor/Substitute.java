 ...
 * For example, if you specify: postprocess=Substitute[name=INPUT_FILE] then all instances of 
 * ${INPUT_FILE} in the template are replaced. You can do string manipulation before replacing.
 * The only one supported at the moment is basename: if you have ${INPUT_FILE}[basename] specified for the
 * string you want replaced, this class calculates the filename of the value of this element and fills it in.
 * 
...
public class Substitute extends ElementPostprocessor {
	...
	public void process(String fqan) throws PostProcessException {
		...
		if (variableName == null || "".equals(variableName) )
				throw new PostProcessException("No name specified to for replacement. Aborting postprocessing.", null);
		
		substituteVariable(template.getTemplateDocument(), variableName);
		
	}
	
	private void substituteVariable(Document jsdl, String name) throws PostProcessException {
...
//					+ e.getMessage());
//			throw new PostProcessException("Could not transform jsdl to string.", e);
//		}
		...
		try {
			...
			myLogger.warn("No substitute attributes for this variable in jsdl file.");
			return;
		}
		...
		for ( Element substituteElement : substituteElements ) {
			...
			String variableName = substituteElement.getAttribute(SUBSTITUTE_VARIABLE_ATTRIBUTE_NAME);
			String configText = null;
			int configStartIndex = variableName.indexOf("[");
			if ( configStartIndex != -1 ) {
				int configEndIndex = variableName.indexOf("]");
				if ( configEndIndex == -1 )
					throw new PostProcessException("Could not find end tag (\"]\") for substitute variable: "+variableName, null);
				configText = variableName.substring(configStartIndex+1, configEndIndex);
				variableName = variableName.substring(0, configStartIndex);
				myLogger.debug("Config string for substitute variable \""+variableName+"\": "+configText);
			}
			
			if ( name.equals(variableName) ) {
				// do the substitution
				try {
				String calculatedReplacement = calculateReplacementString(element.getTextContent(), configText);
				
				substituteElement.setTextContent(calculatedReplacement);
				} catch (Exception e) {
					throw new PostProcessException("Could not calculate replacement string for string \""+element.getTextContent()+"\" using method \""+configText+"\"", e);
				}
...
//				substituteElement.removeAttribute(SUBSTITUTE_VARIABLE_ATTRIBUTE_NAME);
	...
	private String calculateReplacementString(String replacement, String config) {
		...
		if ( BASENAME_KEY.equals(config) ) {
			...
			myLogger.debug("Calculated replacement for \""+replacement+"\" using method \""+config+"\": "+prefixed);
			return prefixed;
	...
	private String basename(String replacement) {
		String basename = replacement.substring(replacement.lastIndexOf("/") + 1);
		return basename;
	}
