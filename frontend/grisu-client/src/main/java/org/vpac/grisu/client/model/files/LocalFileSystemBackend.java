...
public class LocalFileSystemBackend implements FileSystemBackend {
	...
	static final Logger myLogger = Logger.getLogger(LocalFileSystemBackend.class
			.getName());
	...
	public LocalFileSystemBackend(EnvironmentManager em, URI rootUri, String alias) {
		this.em = em;
		this.alias = alias;
...
//			System.out.println(rootUri.toString());
	...
	private File getFileObjectFromBackendFileObject(GrisuFileObject grisuFileObject) {
	...
	public String getPathRelativeToRoot(GrisuFileObject file) {
		if ( isInFileSystem(file) ) {
			if ( isRoot(file) ) {
				...
				return file.getURI().toString().substring(rootUri.toString().length());
			}
		} else {
			return null;
		}
...
//		}
	...
	private URI stripTrailingFileSeperator(URI uri) {
		...
		if ( uri_string.endsWith(File.separator) || uri_string.endsWith("/") ) {
			try {
				String newString = uri.toString().substring(0, uri_string.length()-1);
