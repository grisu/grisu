...
public class TemplateEditPanel extends JPanel implements
	...
	private void setJobDescriptions() {
		if ((template != null) && (template.getJobSubmissionObject() != null)) {
			...
			Method createDescMethod = null;
			try {

				Class gt4SubClass = Class
						.forName("org.vpac.grisu.backend.model.job.gt4.GT4Submitter");

				createDescMethod = gt4SubClass.getMethod(
						"createJobSubmissionDescription",
						InformationManager.class, String.class);

			} catch (Exception e) {
				getGt4TextArea()
						.setText(
								"Could not create rsl. Probably because localBackend jar is not in classpath.");
			}

			if (createDescMethod != null) {
				try {

					String gt4rsl = (String) createDescMethod.invoke(null,
							informationManager,
							SeveralXMLHelpers.fromString(jsdl));

					// String gt4rsl =
					// GT4Submitter.createJobSubmissionDescription(
			...
			try {
				...
				Class gt5SubClass = Class
						.forName("org.vpac.grisu.backend.model.job.gt5.GT5Submitter");

				createDescMethod = gt5SubClass.getMethod(
						"createJobSubmissionDescription",
						InformationManager.class, String.class);
			...
			if (createDescMethod != null) {
				try {
					...
					String gt5rsl = (String) createDescMethod.invoke(null,
							informationManager,
							SeveralXMLHelpers.fromString(jsdl));
