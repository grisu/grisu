...
public class ServiceInterfaceFactory {
	 ...
	 * @throws ServiceInterfaceException if the serviceInterface couldn't be created
	 */
	public static ServiceInterface createInterface(LoginParams params) throws ServiceInterfaceException {
		return createInterface(
				params.getServiceInterfaceUrl(), params.getMyProxyUsername(), params.getMyProxyPassphrase(),
	...
	/**
	 * Creates a ServiceInterface using the url that is provided. Most likely you want a ServiceInterface to a webservice.
	 * But it's also possible to create ServiceInterface that's on your local computer (not at the moment, though - if you are interested in this
	 * contact me via email: markus@vpac.org)
	...
	public static ServiceInterface createInterface(String interfaceUrl,
			...
			char[] httpProxyPassword) throws ServiceInterfaceException {
		...
		for ( String className : KNOWN_SERVICE_INTERFACE_CREATORS ) {
			...
			try {
				serviceInterfaceCreatorClass = Class.forName("org.vpac.grisu.client.control."+className);
			} catch (ClassNotFoundException e) {
				myLogger.warn("Could not find serviceInterfaceCreator class: "+className);
			...
			try {
				serviceInterfaceCreator = (ServiceInterfaceCreator)serviceInterfaceCreatorClass.newInstance();
			} catch (Exception e) {
			...
			if ( ! serviceInterfaceCreator.canHandleUrl(interfaceUrl) ) {
				myLogger.debug(className+" doesn't handle url: "+interfaceUrl+". Trying next serviceInterfaceCreator...");
				continue;
			}
		...
		for ( String name : failedCreators.keySet() ) {
			failedOnes.append(name+": "+failedCreators.get(name).getLocalizedMessage()+"\n");
		}
		
		if ( failedCreators.size() == 1 ) {
			String key = failedCreators.keySet().iterator().next();
			throw new ServiceInterfaceException(failedCreators.get(key).getLocalizedMessage(), failedCreators.get(key));
		} else if ( failedCreators.size() == 0 ) {
			throw new ServiceInterfaceException("Could not find a ServiceInterfaceCreator for this kind of URL.", null);
		} else {
			throw new ServiceInterfaceException("Could not find a single ServiceInterfaceCreator that worked. Tried these:\n"+failedOnes.toString(), null);
		}
